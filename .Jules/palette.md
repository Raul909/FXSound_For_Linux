...
## 2024-04-14 - Custom Slider Accessibility
**Learning:** Custom UI components built with `<div>` (e.g., sliders) must implement full a11y: `role="slider"`, dynamic `tabIndex`, ARIA boundaries, W3C-compliant keyboard handlers (Arrows, Home for min, End for max, preventing default scroll), and explicit `:focus-visible` styling.
**Action:** When creating custom interactive inputs, always provide keyboard navigation parity and explicit focus indicators for non-mouse users.
