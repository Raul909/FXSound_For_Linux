...
## 2026-04-15 - [Unbounded HashMap Insertion DoS]
**Vulnerability:** The `AudioEngine::set_effect` method accepted arbitrary effect names from the frontend and inserted them into a backend `HashMap` without validation, exposing a memory exhaustion (DoS) vulnerability.
**Learning:** Unconstrained inserts into dynamically growing collections (like HashMaps) using unsanitized user input can allow attackers to exhaust system memory.
