...
## 2026-05-25 - Prevent IPC Memory Exhaustion DoS
**Vulnerability:** The `set_effect` command inserted untrusted string keys from the frontend directly into a dynamically sized `HashMap` without validation, creating a memory exhaustion DoS risk.
**Learning:** IPC command parameters in Tauri must be treated as untrusted user input.
**Prevention:** Implement an allow-list for known keys before insertion to prevent bounded memory growth.
