...
## 2026-06-15 - IPC Memory Exhaustion DoS via Untrusted Input
**Vulnerability:** The Tauri command `set_effect` accepted arbitrary strings from the frontend and inserted them directly into a dynamically sized `HashMap` without validation. A malicious actor exploiting the IPC could send thousands of unique string keys, leading to unrecoverable memory exhaustion (Denial of Service).
**Learning:** In Tauri applications, Inter-Process Communication (IPC) command parameters must always be treated as untrusted user input, even if the frontend only sends predefined keys under normal operation.
**Prevention:** Always validate and allow-list string parameters before inserting them into unbounded collections like HashMaps.
