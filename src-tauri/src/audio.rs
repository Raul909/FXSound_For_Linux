...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        let fid_amt = fidelity / 100.0;
        let fid_gain = 1.0 + fid_amt * 0.5;
        let fid_mix1 = 1.0 - fid_amt * 0.3;
        let fid_mix2 = fid_amt * 0.3;

        let has_dynamic = dynamic > 0.0;
        let dyn_thresh = 0.7 - (dynamic / 100.0) * 0.3;
        let dyn_ratio = 0.5 + (1.0 - dynamic / 100.0) * 0.5;

        let has_bass = bass > 0.0;
        let bass_boost = 1.0 + (bass / 100.0) * 0.3;

        // Loop fusion: process all active effects in a single pass
        for sample in buffer.iter_mut() {
            let mut s = *sample;

            if has_fidelity {
                let saturated = (s.abs() * fid_gain).tanh() * s.signum();
                s = s * fid_mix1 + saturated * fid_mix2;
            }

            if has_dynamic && s.abs() > dyn_thresh {
                let sign = s.signum();
                let excess = s.abs() - dyn_thresh;
                s = sign * (dyn_thresh + excess * dyn_ratio);
            }
