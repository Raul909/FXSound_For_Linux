...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        let d_ratio = 0.5 + (1.0 - dynamic / 100.0) * 0.5; // 2:1 at max
        ...
        for sample in buffer.iter_mut() {
            ...
            if fidelity > 0.0 {
                ...
                let saturated = (s.abs() * (1.0 + f_amount * 0.5)).tanh() * s.signum();
                s = s * (1.0 - f_amount * 0.3) + saturated * (f_amount * 0.3);
            }

            // Dynamic compression: reduces the gap between loud and quiet
            if dynamic > 0.0 {
                if s.abs() > d_threshold {
                    let sign = s.signum();
                    let excess = s.abs() - d_threshold;
                    s = sign * (d_threshold + excess * d_ratio);
                }
            }
