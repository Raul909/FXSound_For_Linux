...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        // Early return if no effects are active to prevent redundant memory operations
        if fidelity <= 0.0 && dynamic <= 0.0 && bass <= 0.0 {
            return;
        }

        // Pre-calculate constants outside the loop
        let apply_fidelity = fidelity > 0.0;
        let fid_gain = 1.0 + (fidelity / 100.0) * 0.5;
        let fid_dry = 1.0 - (fidelity / 100.0) * 0.3;
        let fid_wet = (fidelity / 100.0) * 0.3;
        ...
        // Fused loop: Process each sample entirely through all active effects in a single pass
        // Reduces memory reads/writes from O(3N) to O(1N) and improves cache locality
        for sample in buffer.iter_mut() {
            let mut s = *sample;

            if apply_fidelity {
                // Soft saturation — adds harmonics that brighten the sound
                let saturated = (s.abs() * fid_gain).tanh() * s.signum();
                s = s * fid_dry + saturated * fid_wet;
            }
