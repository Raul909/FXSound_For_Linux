...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        // Pre-calculate constants outside the loop to prevent redundant floating-point math
        let fid_amount = fidelity / 100.0;
        let fid_sat_mult = 1.0 + fid_amount * 0.5;
        let fid_mix = fid_amount * 0.3;
        let fid_1m_mix = 1.0 - fid_mix;
        ...
        // to prevent redundant memory reads/writes and cache misses.
        for sample in buffer.iter_mut() {
            ...
            if fidelity > 0.0 {
                ...
                s = s * fid_1m_mix + saturated * fid_mix;
            }
