...
impl AudioEngine {
    ...
    /// Apply audio effects to the buffer using loop fusion.
    fn apply_effects(&self, buffer: &mut [f32]) {
        // Pre-calculate constants to avoid redundant math and map lookups
        ...
        for sample in buffer.iter_mut() {
            ...
            if has_fidelity {
                let saturated = (s.abs() * (1.0 + fid_amount * 0.5)).tanh() * s.signum();
                s = s * (1.0 - fid_amount * 0.3) + saturated * (fid_amount * 0.3);
            }
