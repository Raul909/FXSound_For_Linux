...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        let fid_sat_mult = 1.0 + fid_amount * 0.5;
        let fid_mix_dry = 1.0 - fid_amount * 0.3;
        let fid_mix_wet = fid_amount * 0.3;
        ...
        // Skip processing if no effects are active
        if !has_fidelity && !has_dynamic && !has_bass {
            return;
        }

        // Fused loop: process all active effects in a single pass for cache locality
        for sample in buffer.iter_mut() {
            let mut s = *sample;

            // Fidelity: subtle high-frequency harmonic enhancement
            if has_fidelity {
                let saturated = (s.abs() * fid_sat_mult).tanh() * s.signum();
                s = s * fid_mix_dry + saturated * fid_mix_wet;
            }
