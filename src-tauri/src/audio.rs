...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        let dynamic_prm = self.effects.get("dynamic").filter(|&&d| d > 0.0).map(|d| {
        ...
        if fidelity_amt.is_none() && dynamic_prm.is_none() && bass_boost.is_none() {
            return;
        }
        ...
        for sample in buffer.iter_mut() {
            ...
            if let Some((threshold, ratio)) = dynamic_prm {
                if s.abs() > threshold {
                    let excess = s.abs() - threshold;
                    s = s.signum() * (threshold + excess * ratio);
                }
            }
