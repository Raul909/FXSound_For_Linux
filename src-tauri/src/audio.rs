...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        let fid_inv_mix = 1.0 - fid_mix;
        let fid_drive = 1.0 + fid_amount * 0.5;
        ...
        // Loop fusion: process each sample completely through all active effects
        for sample in buffer.iter_mut() {
            let mut s = *sample;

            if fidelity > 0.0 {
                let saturated = (s.abs() * fid_drive).tanh() * s.signum();
                s = s * fid_inv_mix + saturated * fid_mix;
            }
