...
impl AudioEngine {
    ...
    pub fn process_audio(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        // ⚡ Bolt: Optimized silence detection by using Mean Absolute Value (MAV) approximation
        // instead of RMS. Eliminates expensive per-sample multiplications and global square root calculations.
