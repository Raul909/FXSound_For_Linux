...
impl AudioEngine {
    ...
    pub fn get_fft_data(&self) -> Vec<f32> {
        // SECURITY: Avoid `unwrap()` on `Mutex::lock` to prevent DoS via cascading panics.
        // If the lock is poisoned by another thread panicking, we recover the data
        // rather than taking down the entire application.
...
impl AudioProcessor {
    ...
    fn audio_loop(
        ...
        loop {
            ...
            {
                // SECURITY: Avoid `unwrap()` on `Mutex::lock` to prevent DoS via cascading panics.
                // Recover the lock state safely to maintain audio availability.
