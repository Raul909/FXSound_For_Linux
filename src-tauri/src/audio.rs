...
impl AudioEngine {
    ...
    pub fn process_audio(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        // OPTIMIZATION: Use Mean Absolute Value (MAV) instead of Root Mean Square (RMS)
        // to eliminate expensive per-sample multiplications and global square root calculations.
