...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        // Pre-compute fidelity constants
        let fid_amount = fidelity / 100.0;
        let fid_sat_scale = 1.0 + fid_amount * 0.5;
        let fid_mix = 1.0 - fid_amount * 0.3;
        let fid_sat_mix = fid_amount * 0.3;

        // Pre-compute dynamic constants
        let dyn_threshold = 0.7 - (dynamic / 100.0) * 0.3;
        let dyn_ratio = 0.5 + (1.0 - dynamic / 100.0) * 0.5;

        // Pre-compute bass constants
        let bass_boost = 1.0 + (bass / 100.0) * 0.3;

        // Loop fusion: process each sample through all active effects in a single pass
        for sample in buffer.iter_mut() {
            let mut s = *sample;

            // Fidelity: subtle high-frequency harmonic enhancement
            if has_fidelity {
                let saturated = (s.abs() * fid_sat_scale).tanh() * s.signum();
                s = s * fid_mix + saturated * fid_sat_mix;
            }
            ...
            if has_dynamic {
                ...
                if abs_s > dyn_threshold {
                    s = s.signum() * (dyn_threshold + (abs_s - dyn_threshold) * dyn_ratio);
                }
            }
