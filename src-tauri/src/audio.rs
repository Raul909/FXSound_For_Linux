...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        ...
        let use_fidelity = fidelity > 0.0;
        let use_dynamic = dynamic > 0.0;
        let use_bass = bass > 0.0;

        if !use_fidelity && !use_dynamic && !use_bass {
            return;
        }

        // Pre-calculate constants outside the loop
        let fid_amount = fidelity / 100.0;
        let fid_sat_mul = 1.0 + fid_amount * 0.5;
        ...
        for sample in buffer.iter_mut() {
            ...
            if use_fidelity {
                let saturated = (s.abs() * fid_sat_mul).tanh() * s.signum();
