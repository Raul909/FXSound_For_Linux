...
impl AudioEngine {
    ...
    fn apply_effects(&self, buffer: &mut [f32]) {
        // Pre-calculate constants outside the loop to prevent redundant math
        let fidelity_val = self.effects.get("fidelity").copied().unwrap_or(0.0);
        let dynamic_val = self.effects.get("dynamic").copied().unwrap_or(0.0);
        let bass_val = self.effects.get("bass").copied().unwrap_or(0.0);

        let has_fidelity = fidelity_val > 0.0;
        let has_dynamic = dynamic_val > 0.0;
        let has_bass = bass_val > 0.0;

        // Skip entirely if no effects are applied
        if !has_fidelity && !has_dynamic && !has_bass {
            return;
        }

        let fidelity_amount = fidelity_val / 100.0;
        let dyn_threshold = 0.7 - (dynamic_val / 100.0) * 0.3;
        let dyn_ratio = 0.5 + (1.0 - dynamic_val / 100.0) * 0.5;
        let bass_boost = 1.0 + (bass_val / 100.0) * 0.3;

        // Loop fusion: process each sample entirely through all active effects in a single pass
        for sample in buffer.iter_mut() {
            let mut s = *sample;

            if has_fidelity {
                let saturated = (s.abs() * (1.0 + fidelity_amount * 0.5)).tanh() * s.signum();
                s = s * (1.0 - fidelity_amount * 0.3) + saturated * (fidelity_amount * 0.3);
            }
