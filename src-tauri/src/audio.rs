...
impl AudioEngine {
    ...
    pub fn process_audio(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        // ⚡ Bolt: Optimized silence detection using Mean Absolute Value (MAV)
        // instead of RMS to eliminate expensive per-sample multiplications and square root calculation.
