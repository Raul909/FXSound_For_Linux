...
impl AudioEngine {
    ...
    pub fn process_audio(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        // ⚡ Bolt: Used Mean Absolute Value (MAV) approximation for silence detection
        // (mav < 0.0009) to eliminate expensive per-sample multiplications and global
        // square root calculations compared to RMS (rms.sqrt() < 0.001).
