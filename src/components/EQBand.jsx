...
const EQBand = memo(function EQBand({ freq, value, onChange, disabled }) {
    ...
    function handleKeyDown(e) {
        if (disabled) return;
        if (["ArrowUp", "ArrowRight"].includes(e.key)) { onChange(Math.min(12, value + 1)); e.preventDefault(); }
        if (["ArrowDown", "ArrowLeft"].includes(e.key)) { onChange(Math.max(-12, value - 1)); e.preventDefault(); }
    }
