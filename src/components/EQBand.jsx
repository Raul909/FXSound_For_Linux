...
const EQBand = memo(function EQBand({ freq, value, onChange, disabled }) {
    ...
    function handleKeyDown(event) {
        if (disabled) return;
        let newValue = value;
        if (event.key === "ArrowUp" || event.key === "ArrowRight") newValue = Math.min(12, value + 1);
        else if (event.key === "ArrowDown" || event.key === "ArrowLeft") newValue = Math.max(-12, value - 1);
        else if (event.key === "PageUp") newValue = Math.min(12, value + 3);
        else if (event.key === "PageDown") newValue = Math.max(-12, value - 3);
