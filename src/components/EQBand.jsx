...
const EQBand = memo(function EQBand({ freq, value, onChange, disabled }) {
    ...
    // Handle keyboard interaction on the slider track
    function handleKeyDown(event) {
        if (disabled) return;
        let newValue = value;
        if (event.key === "ArrowRight" || event.key === "ArrowUp") newValue = Math.min(12, value + 1);
        else if (event.key === "ArrowLeft" || event.key === "ArrowDown") newValue = Math.max(-12, value - 1);
        else return;
