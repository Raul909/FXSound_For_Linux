...
const EffectSlider = memo(function EffectSlider({ label, value, onChange, disabled }) {
    ...
    function handleKeyDown(event) {
        ...
        if (["ArrowRight", "ArrowUp"].includes(event.key)) { event.preventDefault(); onChange(Math.min(100, value + step)); }
        if (["ArrowLeft", "ArrowDown"].includes(event.key)) { event.preventDefault(); onChange(Math.max(0, value - step)); }
    }
