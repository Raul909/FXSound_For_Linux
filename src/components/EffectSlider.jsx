...
const EffectSlider = memo(function EffectSlider({ label, value, onChange, disabled }) {
    ...
    function handleKeyDown(e) {
        if (disabled) return;
        if (["ArrowUp", "ArrowRight"].includes(e.key)) { onChange(Math.min(100, value + 1)); e.preventDefault(); }
        if (["ArrowDown", "ArrowLeft"].includes(e.key)) { onChange(Math.max(0, value - 1)); e.preventDefault(); }
    }
