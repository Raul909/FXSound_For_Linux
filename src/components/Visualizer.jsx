...
// Extracted radii arrays for canvas roundRect to avoid GC thrashing in animation loop
const INACTIVE_RADII = [1, 1, 0, 0];
const ACTIVE_RADII = [3, 3, 0, 0];
    ...
    const drawFrame = useCallback((now, lastTimeRef) => {
        ...
        for (let i = 0; i < BAR_COUNT; i++) {
            ...
            if (!isPowered) {
                ...
                ctx.globalAlpha = 0.3;
                ctx.beginPath();
                ctx.roundRect(x, y, barW, barH, INACTIVE_RADII);
                ctx.fill();
                ctx.globalAlpha = 1;
                continue;
            }
            ...
            ctx.beginPath();
            ctx.roundRect(x, y, barW, barH, ACTIVE_RADII);
            ctx.fill();
